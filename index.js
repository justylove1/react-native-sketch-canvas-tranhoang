...
export default class RNSketchCanvas extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      alpha: 'FF',
      onSelectType: [
        { id: 0, image: require('./src/images/stroke1.png'), seleted: false },
        { id: 1, image: require('./src/images/stroke2.png'), seleted: false },
        { id: 2, image: require('./src/images/stroke3.png'), seleted: false },
        { id: 3, image: require('./src/images/stroke4.png'), seleted: false },
        { id: 4, image: require('./src/images/stroke5.png'), seleted: false },
      ]
    }
  ...
  render() {
    return (
      <View style={this.props.containerStyle}>
        ...
        />
        <View style={{ flexDirection: 'column', backgroundColor: '#0B243B', height: 200, paddingHorizontal: 5 }}>
          ...
          <View style={{ backgroundColor: 'transparent', marginTop: 10, marginHorizontal: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
            <TouchableOpacity style={{ alignItems: 'center', backgroundColor: 'transparent', paddingTop: 10, borderRadius: 10, borderWidth: 2, borderColor: this.state.onSelectType[0].seleted ? 'white':'transparent' }}
              ...
              onPress={() => {
                this.setState({
                  ...
                  onSelectType: this.state.onSelectType.map((item) => {
                    if (item.id == 0) {
                      return ({
                        ...item,
                        seleted: true
            ...
            >
              <FastImage
                source={this.state.onSelectType[0].image}
            ...
            <TouchableOpacity style={{
              ...
              onPress={() => {
                this.setState({
                  strokeWidth: this.props.defaultStrokeWidth + 3,
                  onSelectType: this.state.onSelectType.map((item) => {
                    if (item.id == 1) {
                      return ({
                        ...item,
            ...
            >
              <FastImage
                source={this.state.onSelectType[1].image}
                style={{ backgroundColor: 'transparent', width: 60, height: 60, }}
                resizeMode={FastImage.resizeMode.contain}
              />
            ...
            <TouchableOpacity style={{
              ...
              onPress={() => {
                this.setState({
                  strokeWidth: this.props.defaultStrokeWidth + 6,
                  onSelectType: this.state.onSelectType.map((item) => {
                    if (item.id == 2) {
                      return ({
                        ...item,
            ...
            >
              <FastImage
                source={this.state.onSelectType[2].image}
                style={{ backgroundColor: 'transparent', width: 60, height: 60, }}
                resizeMode={FastImage.resizeMode.contain}
              />
            ...
            <TouchableOpacity style={{
              ...
              onPress={() => {
                this.setState({
                  strokeWidth: this.props.defaultStrokeWidth + 9,
                  onSelectType: this.state.onSelectType.map((item) => {
                    if (item.id == 3) {
                      return ({
                        ...item,
            ...
            >
              <FastImage
                source={this.state.onSelectType[3].image}
                style={{ backgroundColor: 'transparent', width: 60, height: 60, }}
                resizeMode={FastImage.resizeMode.contain}
              />
            ...
            <TouchableOpacity style={{
              ...
              onPress={() => {
                this.setState({
                  strokeWidth: this.props.defaultStrokeWidth + 13,
                  onSelectType: this.state.onSelectType.map((item) => {
                    if (item.id == 4) {
                      return ({
                        ...item,
            ...
            >
              <FastImage
                source={this.state.onSelectType[4].image}
                style={{ backgroundColor: 'transparent', width: 60, height: 60, }}
                resizeMode={FastImage.resizeMode.contain}
              />
              {this.props.strokeWidthComponent(this.props.defaultStrokeWidth + 13)}
            </TouchableOpacity>
